...
path.basename('/');
/*Start Login service*/
var url='mongodb+srv://hotshotsolutions:Hotshot@321@cluster0-5if9n.mongodb.net/manis?retryWrites=true&w=majority'
...
router.get('/api/customer',function(req,res){
	...
	MongoClient.connect(url,{ useNewUrlParser: true },function(err,client){
		if(err){
			...
			//console.log(db);
			//var collection=db.collection("login");
			db.collection('customer').find({}).toArray(function (findErr, result) {
				if (findErr) throw findErr;
				console.log(result);
				if(result)
				 res.json(result);
...
var query=req.body;
	...
	MongoClient.connect(url,{ useNewUrlParser: true },function(err,client){
		if(err){
			...
			delete query.id;
			delete query._id;
			db.collection('customer').insert(query, function (findErr, result) {
				if (findErr) throw findErr;
				console.log(result);
				if(result)
				 res.json("Success");
